...
export function StatsCards({ conversations }: StatsCardsProps) {
  ...
  const statCards = [
    ...
    },
    {
      ...
      delta: stats.escalationCount > 0 ? `● ${stats.escalationCount} en attente` : '● 0 en attente',
      deltaColor: stats.escalationCount > 0 ? 'text-amber-600' : 'text-slate-600',
    },
  ]
      ...
      {statCards.map((stat, index) => (
        <Card key={index} className="p-0 rounded-2xl">
          <CardContent className="p-4">
            ...
            <Badge className={`text-xs mt-2 bg-blue-500 ${stat.deltaColor}`}>{stat.delta}</Badge>
          </CardContent>
        </Card>
      ))}
    </div>
