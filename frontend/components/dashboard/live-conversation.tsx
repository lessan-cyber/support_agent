...
export function LiveConversation({ ticketId, onTakeOver }: LiveConversationProps) {
  ...
  useEffect(() => {
    ...
    const fetchConversationMessages = async () => {
      try {
        ...
        console.error('[LiveConversation] Error fetching messages:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  if (!ticketId) {
    return (
      <Card className="p-8 text-center">
        <p className="text-sm text-muted-foreground">Sélectionnez une conversation pour voir les détails</p>
      </Card>
    )
  }
          ...
          messages.map((msg, i) => (
            ...
            >
              <div className="text-xs opacity-70 mb-1 font-semibold capitalize">
                {msg.sender_type === 'admin' ? 'Agent' : msg.sender_type}
              </div>
              <p>{msg.content}</p>
            </div>
          ))
