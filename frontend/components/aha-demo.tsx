...
export function AhaDemo() {
    ...
    const [setHumanRequested] = useState(false);
    ...
    const [liveConvMessages, setLiveConvMessages] = useState<
        Array<{ role: string; from: string; text: string }>
    >([]);
    ...
    const startConversation = () => {
        ...
        addTimeout(() => {
            addMsg(
                "bot",
                "Bonjour 👋 Bienvenue sur ShopIA ! Testez-moi — posez-moi une question sur notre politique de retour, ou demandez à parler à un humain.",
            );
            setQuickReplies([
        ...
        const next = () => {
            if (i < replies.length) {
                ...
                if (!dashboardUpdated) {
                    ...
                    addTimeout(triggerDashboardUpdate, 1200);
                }
                setQuickReplies([
                    { label: "📊 Voir le dashboard →", key: "showdash" },
                ]);
            }
        };
        next();
    ...
    const sendUserMsg = () => {
        ...
        if (lower.includes("retour") || lower.includes("remboursement")) {
            addTimeout(handleRetour, 300);
        } else if (
            lower.includes("humain") ||
            lower.includes("agent") ||
            lower.includes("personne")
        ) {
            addTimeout(handleHumain, 300);
        } else {
            addTimeout(() => {
                addMsg(
                    "bot",
                    "Je peux vous aider avec notre politique de retour ou vous connecter à un agent humain. Que souhaitez-vous ?",
                );
                setQuickReplies([
                    { label: "↩ Politique de retour", key: "retour" },
                    { label: "👤 Parler à un humain", key: "humain" },
        ...
        const convo = [
            {
                role: "from-user",
                from: "Visiteur",
                text: "Bonjour, politique de retour ?",
            },
            {
                role: "from-bot",
                from: "ShopIA IA",
                text: "Les articles doivent être dans leur état d'origine. Remboursement sous 5 jours.",
            },
            {
                role: "from-user",
                from: "Visiteur",
                text: "Ok. Je voudrais parler à un humain.",
            },
            {
                role: "from-bot",
                from: "ShopIA IA",
                text: "Bien sûr, transfert en cours... 🔔",
            },
        ];
        let i = 0;
        const addConvMsg = () => {
    ...
    const agentTakeover = () => {
        setLiveConvMessages((prev) => [
            ...prev,
            {
                role: "from-human-agent",
                from: "Vous (Agent)",
                text: "Bonjour Alex, je prends en charge votre demande. Comment puis-je vous aider ?",
            },
        ]);
                    ...
                    <div className="flex-1 min-w-56 bg-background rounded-2xl p-6 min-h-96 relative">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-4">
                            {[0, 1, 2].map((i) => (
                                <div
                                    key={i}
                                    className="bg-ring/30 rounded-lg p-2"
                                >
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-3 gap-3">
                        {[
                            ...
                            },
                            {
                                ...
                                value: dashboardUpdated ? "5" : "4",
                                delta: dashboardUpdated
                                    ? "● 1 en attente"
                                    : "● 0 en attente",
                                deltaColor: dashboardUpdated
                                    ? "text-amber-600"
                                    : "text-slate-600",
                            },
                        ].map((stat, i) => (
                            <div
                                key={i}
                                className="bg-background/50 border border-ring/50 rounded-lg p-3"
                            >
                                ...
                                <div
                                    className={`text-xs mt-1 ${stat.deltaColor || "text-green-600"}`}
                                >
                                    {stat.delta}
                                </div>
                            </div>
                    ...
                    <div className="bg-background/50 border border-ring/50 rounded-2xl overflow-hidden">
                        <div className="px-3.5 py-2.5 border-b border-ring/50 flex items-center justify-between">
                            ...
                            {dashboardUpdated && (
                                <span className="text-xs font-semibold bg-red-50 text-red-900 px-2 py-1 rounded-full">
                                    ● Nouveau
                                </span>
                            )}
                        </div>
                                ...
                                <div className="flex gap-2.5 items-start">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-xs text-muted-foreground mt-1">
                                            Délai de livraison pour la Corse ?
                                        </div>
                                        <div className="text-xs text-muted-foreground mt-1">
                                            il y a 8 min
                                        </div>
                            ...
                            <div className="p-3 hover:bg-foreground/10 transition-colors">
                                <div className="flex gap-2.5 items-start">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-xs text-muted-foreground mt-1">
                                            Comment suivre ma commande #4521 ?
                                        </div>
                                        <div className="text-xs text-muted-foreground mt-1">
                                            il y a 22 min
                                        </div>
